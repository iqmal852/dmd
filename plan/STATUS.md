...
| 2026-09-21 | 00 | Passkeys removed after all (2FA kept) — Fortify feature flag, config block, `User`'s `PasskeyAuthenticatable`/`PasskeyUser`, the `passkeys` table/migration, and every passkey UI surface (login, confirm-password, security settings) | Explicit user request — one admin, one authenticator app for 2FA is enough; passkeys were never load-bearing, just the starter kit default |
| 2026-09-21 | 03 | Public landing page (`/`, `welcome.tsx`, Picture1.png panels 5–6) removed; `/` now redirects straight to `/admin/stations` | Explicit user request: this is an internal-use tool, not a public site — no visitor is expected to land on the bare domain root, only on `/d/{id}` via a QR scan or directly into the admin console |
